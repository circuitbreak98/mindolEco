 ...
 */
public class BASBCO00700 extends BaseBizUnit {
	...
	public IDataSet getProductList(IDataSet requestData,
		...
		if (log.isDebugEnabled()) {
			log.debug("BASBCO00700.getProductList method start >>>");
		}

		IRecordSet rs = queryForRecordSet("BASBCO00700.getProductList",
		...
		//responseData.putRecordSet("ds_DealCo", rs);
		return responseData;
	}
	...
	public IDataSet getProdList(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		Log log = LogManager.getLog(onlineCtx);
		if (log.isDebugEnabled()) {
			log.debug("BASBCO00700.getProdList method start >>>");
		}

		IRecordSet rs = queryForRecordSet("BASBCO00700.getProdList",
				requestData.getFieldMap(), onlineCtx);
