 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2010-07-13 10:42:24</li>
 * </ul>
 *
 * @author 김연석 (kimyeunsuk)
 */
public class BASUSM00800 extends BaseBizUnit {
	...
	private static final String userPSnM   = "N";
	private static final String userSale   = "S";
	/**
	 * 
	 *
	 * @author 김연석 (kimyeunsuk)
	 ...
	 */
	public IDataSet getMappingUserList(IDataSet requestData, IOnlineContext onlineCtx) {

		Log log = LogManager.getLog(onlineCtx);
		if (log.isDebugEnabled()) {
			log.debug("getMappingUserList method start");
		}

		// 판매점사용자 매핑관리(BASUSM00700) 화면과 PS&M정직원 매핑관리(BASUSM00900) 화면에서 같이 사용하므로
		// 호출한 화면의 성격에 따라 Query 부분을 나눈다.
		IRecordSet  rs = null;

        String jobFlag = requestData.getField("D14_YN").toString();
		log.debug("D14_YN : [" + jobFlag + "]");

        if (userD14.equals(jobFlag)) {
    		rs = queryForRecordSet("BASUSM00800.getMappingUserList", requestData.getFieldMap());
        } 
        if (userPSnM.equals(jobFlag)) {
    		rs = queryForRecordSet("BASUSM00800.getMappingPsUser", requestData.getFieldMap());
        }
        if (userSale.equals(jobFlag)) {
    		rs = queryForRecordSet("BASUSM00800.getMappingSaleUser", requestData.getFieldMap());
        }
		...
		IDataSet responseData = DataSetFactory.createWithOKResultMessage(BaseConstants.QUERY_OK_MESSAGE_ID, new String[] { String
	 ...
	 * @return 요청처리 완료후 작성된 응답정보를 Wrapping하고 있는 DataSet 객체
	 */
	public IDataSet saveMappingUser(IDataSet requestData, IOnlineContext onlineCtx) {

		Log log = LogManager.getLog(onlineCtx);
		if (log.isDebugEnabled()) {
			log.debug("saveMappingUser method start");
		}
		...
		IRecordSet rsPortalUser  = requestData.getRecordSet("Portal");

		String jobFlag       = rsPortalUser.get(0, "D14_YN").toString();
		String strPortalID   = rsPortalUser.get(0, "PORTAL_ID") == null ? "Portal_ID" : rsPortalUser.get(0, "PORTAL_ID").toString();
		...
		for (int i = 0; i < rsMappingUser.getRecordCount(); i++) {
			...
			strUserID       = oMapRowData.get("USER_ID").toString();
			strJobFlag      = oMapRowData.get("CHK")   == null ? "0" : oMapRowData.get("CHK").toString();

			if (MappinFlag.equals(strJobFlag)) {
				oMapRowData.put("PORTAL_ID", strPortalID);
				oMapRowData.put("USER_ID",   strUserID);

				// 판매점 사용자 매핑의 경우만 TBAS_PORTAL_MAPPING_RECV에 처리되었음을 Update...
                if (userD14.equals(jobFlag)) {
    				updateCnt = update("BASUSM00800.saveMappingRecv", oMapRowData, onlineCtx);
    				updateCount++;
                }

                // PORTAL_USER_ID / PWD Update...
				updateCnt = update("BASUSM00800.saveMappingUser", oMapRowData, onlineCtx);
				updateCount++;

				// MQ를 통해 판매점 포탈로 I/F 전송

				mqLoginID     = strPortalID;
				mqSysJobID    = oMapRowData.get("USER_ID").toString();
				mqOrgID       = oMapRowData.get("ORG_ID")        == null ?"" : oMapRowData.get("ORG_ID").toString();
		        mqRelOrgID    = oMapRowData.get("REL_ORG_ID")    == null ?"" : oMapRowData.get("REL_ORG_ID").toString();
		        mqUserTypeCd  = oMapRowData.get("USER_TYPE_CD")  == null ?"" : oMapRowData.get("USER_TYPE_CD").toString();
		        mqHndStsCd    = oMapRowData.get("HND_STS_CD")    == null ?"" : oMapRowData.get("HND_STS_CD").toString();
		        ...
		        mqSysCl       = oMapRowData.get("SYS_CL")        == null ?"" : oMapRowData.get("SYS_CL").toString();
	...
	private static String lPad(String sStr, int size, String fStr){
		byte[] b = sStr.getBytes();
		int  len = b.length;
		int  tmp = size - len;
