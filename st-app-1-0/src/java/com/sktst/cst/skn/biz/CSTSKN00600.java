 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2012-06-28 15:45:17</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN00600 extends BaseBizUnit {
	...
	public IDataSet getPrchsBuyList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTSKN00600.getPrchsBuyList",
		...
		IRecordSet dtl = queryForRecordSet("CSTSKN00600.getPrchsDtlBuyList",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00600.updatePrchsMgmtCancel",
							rs.getRecord(i), onlineCtx);
					update("CSTSKN00600.updatePrchsDtlCancel", rs.getRecord(i),
							onlineCtx);
					update("CSTSKN00600.updateConsultMgmtCancel", rs
		...
		if (rs1 != null) {
			...
			for (int i = 0; i < rs1.getRecordCount(); i++) {
				if (rs1.get(i, "PRC_CHECK").equals("1")) {
					...
					iMaster.put("pos_agency_id", rs1.get(i, "POS_AGENCY")
							.toString());
					iMaster.put("prchs_plc_id", rs1.get(i, "CON_PLC_CD")
							.toString());
					iMaster.put("in_cl", "114");
					iMaster.put("in_fix_qty", "1");
					iMaster.put("PRCHS_MGMT_NO", rs1.get(i, "PRCHS_MGMT_NO")
							.toString());
					...
					iProd.put("prod_cl", "1");
					iProd.put("in_qty", "1");
					if (rs1.get(i, "CON_PLC_CD").toString().equals("11050")){
						iProd.put("in_amt", rs1.get(i, "SKN_PRCHS_AMT").toString());
					} else {
						iProd.put("in_amt", rs1.get(i, "PRCHS_AMT").toString());
					}
					iProd.put("eqp_st", rs1.get(i, "EQP_ST").toString());
					iProd.put("PRCHS_MGMT_NO", rs1.get(i, "PRCHS_MGMT_NO")
							.toString());
					...
					for (int j = 0; j < rs2.getRecordCount(); j++) {
						if (rs1.get(i, "PRCHS_MGMT_NO").equals(
								rs2.get(j, "PRCHS_MGMT_NO").toString())) {
					...
					update("CSTSKN00600.updatePrchsMgmtIn", rs1.getRecord(i),
							onlineCtx);

					INN inn = (INN) lookupBizComponent("sktst.dis.INN");
					if (rs1.get(i, "SKN_OP_YN").equals("N")) {
						try {
							IDataSet dsSaveInn = inn.saveInn(requestData,
									onlineCtx);
						} catch (RemoteException rEx) {
							if (log.isDebugEnabled()) {
								log
										.debug("savePrchs insert RemoteException rEx: "
												+ rEx.getMessage());
							}
							...
							if (log.isDebugEnabled()) {
								log.debug("savePrchs insert Exception ex: "
										+ ex.getMessage());
							}
							throw ex;
						}
						...
						if (rs1.get(i, "SKN_OP_YN").equals("Y")) {
							try {
								IDataSet dsInnCncl = inn.saveInnCncl(
										requestData, onlineCtx);
							} catch (RemoteException rEx) {
								if (log.isDebugEnabled()) {
									log
											.debug("savePrchs delete RemoteException rEx: "
													+ rEx.getMessage());
								}
								throw new BizRuntimeException(
										rEx
								...
								if (log.isDebugEnabled()) {
									log.debug("savePrchs delete Exception ex: "
											+ ex.getMessage());
								}
								throw ex;
							}
	...
	public IDataSet getPrchsBuyCList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTSKN00600.getPrchsBuyCList",
				requestData.getFieldMap());
