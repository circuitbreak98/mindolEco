 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2012-10-04 10:18:38</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN00900 extends BaseBizUnit {
	...
	public IDataSet getPrchsRemitSknList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTSKN00900.getPrchsRemitSknList",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00900.updateSknRemitYes", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateSknRemitNo(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00900.updateSknRemitNo", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
