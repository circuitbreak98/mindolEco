 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2013-11-19 14:02:29</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN01300 extends BaseBizUnit {
	...
	public IDataSet getDisCancel(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		IRecordSet rs = queryForRecordSet("CSTSKN01300.getDisCancel",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN01300.deleteDis", rs.getRecord(i), onlineCtx);
					update("CSTSKN01300.deleteProdDis", rs.getRecord(i),
							onlineCtx);
					update("CSTSKN01300.deleteDisAmt", rs.getRecord(i),
							onlineCtx);
					update("CSTSKN01300.updatePrchsPrcSt", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
