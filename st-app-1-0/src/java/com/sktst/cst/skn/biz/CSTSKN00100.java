 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2012-06-19 14:52:52</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN00100 extends BaseBizUnit {
	...
	public IDataSet getConsultInList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTSKN00100.getConsultInList",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00100.updateConsultInNo", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateConsultInYes(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00100.updateConsultInYes", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateConsultInRmk(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN00100.updateConsultInRmk", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
