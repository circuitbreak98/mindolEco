 ...
 * <li>업무 그룹명 : </li>
 * <li>설 명 : </li>
 * <li>작성일 : 2014-09-30 14:20:45</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN01400 extends BaseBizUnit {
	...
	public IDataSet getSknAsianaMilList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTSKN01400.getSknAsianaMilList",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN01400.updateSknMilYes", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateSknMilNo(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN01400.updateSknMilNo", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
