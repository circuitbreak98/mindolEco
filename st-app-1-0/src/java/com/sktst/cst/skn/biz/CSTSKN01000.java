 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2012-10-04 15:17:49</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTSKN01000 extends BaseBizUnit {
	...
	public IDataSet getPrchsChagDedtSknList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet(
				"CSTSKN01000.getPrchsChagDedtSknList", requestData
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN01000.updateSknChagDedtYes", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateSknChagDedtNo(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTSKN01000.updateSknChagDedtNo", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
