 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2012-03-14 13:54:05</li>
 * </ul>
 *
 * @author 전미량 (jeonmiryang)
 */
public class CSTRET00100 extends BaseBizUnit {
	...
	public IDataSet savePrchsMgmtRtn(IDataSet requestData,
		...
		if (log.isDebugEnabled()) {
			log.debug("CSTRET00100.savePrchsMgmtRtn method start");
		}
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				...
				if (mSave.get("CHK").equals("1")) {
					update("CSTRET00100.updatePrchsMgmtRtn", mSave, onlineCtx);
				}
			}
		}
	 ...
	 * @return 요청처리 완료후 작성된 응답정보를 Wrapping하고 있는 DataSet 객체
	 */
	public IDataSet getBoxPrchsLst(IDataSet requestData, IOnlineContext onlineCtx)
			throws Exception {

		Log log = LogManager.getLog(onlineCtx);
		if (log.isDebugEnabled()) {
			log.debug("CSTRET00100.getBoxPrchsLst method start >>>");
		}

		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTRET00100.getBoxPrchsLst",
