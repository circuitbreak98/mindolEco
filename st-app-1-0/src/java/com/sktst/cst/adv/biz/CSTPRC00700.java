 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2011-11-14 10:15:21</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTPRC00700 extends BaseBizUnit {
	...
	public IDataSet getPrchsChagDedtList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTPRC00700.getPrchsChagDedtList",
		...
		IRecordSet dtl = queryForRecordSet("CSTPRC00700.getPrchsDtlDedtList",
				requestData.getFieldMap(), onlineCtx);
...
//log.debug("requestData : " + requestData);
					...
					update("CSTPRC00700.updatePrchsChagDedtOp", rs1.getRecord(i),
							onlineCtx);
						
					INN inn = (INN) lookupBizComponent("sktst.dis.INN");
					if (rs1.get(i, "CHAG_DEDT_OP_YN").equals("N")) {
						try {
						...
						if (rs1.get(i, "CHAG_DEDT_OP_YN").equals("Y")){
							try {
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTPRC00700.updatePrchsChagDedtOp", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
