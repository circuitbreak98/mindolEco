 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2011-09-07 13:31:29</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTADV00800 extends BaseBizUnit {
	...
	public IDataSet getConsultInList(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTADV00800.getConsultInList",
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTADV00800.updateConsultInYes", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateConsultInNo(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTADV00800.updateConsultInNo", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
	...
	public IDataSet updateConsultInRmk(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "PRC_CHECK").equals("1")) {
					update("CSTADV00800.updateConsultInRmk", rs.getRecord(i),
							onlineCtx);
				}
			}
		}
