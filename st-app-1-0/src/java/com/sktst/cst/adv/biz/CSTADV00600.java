 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2011-08-18 15:35:23</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTADV00600 extends BaseBizUnit {
	...
	public IDataSet getSerNumCheck(IDataSet requestData,
		...
		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTADV00600.getSerNumCheck",
		...
		responseData.putRecordSet("ds_ser_num_check", rs);
		return responseData;
	...
	public IDataSet saveConsultMgmtXls(IDataSet requestData,
		...
		log.debug("saveConsultMgmtXls method start");
		...
		if (xls != null) {
			for (int i = 0; i < xls.getRecordCount(); i++) {
				...
				mPrchsNo = (Map) queryForObject("CSTADV00200.getPrchsMgmtNo",
						mMgmt, onlineCtx);
				//				mProdDtl = (Map) queryForObject("CSTADV00200.getProdDtl",
				//						mMgmt, onlineCtx);
				...
				for (int j = 0; j < rsProdDtl.getRecordCount(); j++) {
					...
					mProdDtl.put("CON_MGMT_NO", mConNo.get("CON_MGMT_NO"));
					mProdDtl
							.put("PRCHS_MGMT_NO", mPrchsNo.get("PRCHS_MGMT_NO"));
					...
					insert("CSTADV00600.addPrchsDtlXls", mProdDtl, onlineCtx);
					//					log.debug("mProdDtl : " + mProdDtl);
					//					if ( !sDtlCl.equals("Y")){
					//						sOther = sOther
					//					}
				}
				...
				insert("CSTADV00600.addConsultMgmtXls", mMgmt, onlineCtx);
				insert("CSTADV00600.addPrchsMgmtXls", mMgmt, onlineCtx);
	...
	public IDataSet getErrorCheckXls(IDataSet requestData,
		...
		if (rsXlsList != null) {
			 ...
			 * 1. 모델조회.
			 *********************************************/			
			IRecordSet rsProdList = queryForRecordSet("CSTADV00600.getProdList", requestData
			 ...
			 * 2. 색상조회.
			 *********************************************/			
			IRecordSet rsColorList = queryForRecordSet("CSTADV00600.getColorList", requestData
					.getFieldMap(), onlineCtx);
			 ...
			 * 3. 거래처조회.
			 *********************************************/			
			IRecordSet rsDealList = queryForRecordSet("CSTADV00600.getDealList", requestData
					.getFieldMap(), onlineCtx);
			 ...
			 * 4. 단말기 등급 조회.
			 *********************************************/			
			IRecordSet rsGradeList = queryForRecordSet("CSTADV00600.getGradeList", requestData
					.getFieldMap(), onlineCtx);
			 ...
			 * 5. 은행코드 조회.
			 *********************************************/			
			IRecordSet rsBankList = queryForRecordSet("CSTADV00600.getBankList", requestData
					.getFieldMap(), onlineCtx);
			...
			for(int k = 0 ; k < rsBankList.getRecordCount() ; k++){
				...
				mAllBank.put(mBank.get("comm_cd_val"), mBank);
			}
			...
			for (int i = 0; i < rsXlsList.getRecordCount(); i++) {
				...
				rsConsultList = queryForRecordSet("CSTADV00600.getConsultMgmt",
						mProdMap, onlineCtx);
...
//		IRecordSet xls = requestData.getRecordSet("ds_upLoadXlsList");
		...
		if (xls != null) {
			for (int i = 0; i < xls.getRecordCount(); i++) {
				...
				mMgmt = xls.getRecordMap(i);
				String buyCl = mMgmt.get("BUY_CL") == null ? "" : mMgmt.get("BUY_CL").toString().replaceAll(" ", "");
				
				if (buyCl.equals("05")) {
					update("CSTADV00600.updatePrhcsMgmtXlsY", mMgmt, onlineCtx);
				} else if (buyCl.equals("06")) {
					update("CSTADV00600.updatePrhcsMgmtXlsN", mMgmt, onlineCtx);
				}
