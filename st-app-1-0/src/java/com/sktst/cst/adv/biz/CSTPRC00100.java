 ...
 * <li>설 명 : </li>
 * <li>작성일 : 2011-07-20 19:43:09</li>
 * </ul>
 *
 * @author 민동운 (mindongwoon)
 */
public class CSTPRC00100 extends BaseBizUnit {
	 ...
	 */
	public IDataSet getPrchsList(IDataSet requestData, IOnlineContext onlineCtx) throws Exception {

		Log log = LogManager.getLog(onlineCtx);

		//질의를 수행한다.
		IRecordSet rs = queryForRecordSet("CSTPRC00100.getPrchsList",
		...
		IRecordSet dtl = queryForRecordSet("CSTPRC00100.getPrchsDtlRemitList",
				requestData.getFieldMap(), onlineCtx);
	...
	public IDataSet deletePrchsMgmt(IDataSet requestData,
		...
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "DEL_CHECK").equals("1")) {
					update("CSTPRC00100.updatePrchsMgmt", rs.getRecord(i),
							onlineCtx);
					update("CSTPRC00100.updateConsultMgmtBuyCl", rs
	...
	public IDataSet updatePrchsMgmtIn(IDataSet requestData,
		...
		if (iTime >= 160000 ) {
			if (minData.get("CUSER").toString().equals("SC02400169") ||
				...
				minData.get("CUSER").toString().equals("SC2102463") ||
				minData.get("CUSER").toString().equals("ECO00007") ||
				minData.get("CUSER").toString().equals("ECO00003") ||
				minData.get("CUSER").toString().equals("ECO00004") ||
...
//log.debug("requestData : " + requestData);
					...
					update("CSTPRC00100.updatePrchsMgmtIn", rs1.getRecord(i),
							onlineCtx);
					
					INN inn = (INN) lookupBizComponent("sktst.dis.INN");
					if (rs1.get(i, "REMIT_YN").equals("N")) {
						try {
						...
						if (rs1.get(i, "REMIT_YN").equals("Y")){
							try {
...
/*		
		if (rs != null) {
			for (int i = 0; i < rs.getRecordCount(); i++) {
				if (rs.get(i, "DEL_CHECK").equals("1")) {
					update("CSTPRC00100.updatePrchsMgmtIn", rs.getRecord(i), onlineCtx);
				}
			}
		}
