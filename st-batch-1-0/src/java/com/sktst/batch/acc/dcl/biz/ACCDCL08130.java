 ...
 * <li>설 명 : 부가서비스수수료 일마감 </li>
 * <li>작성일 : 2009-04-27</li>
 * </ul>
 *
 * @author 이영진 (leeyoungjin)
 */
public class ACCDCL08130 extends AbsBatchJobBiz {

	private static final String PROG_ID = "ACCDCL08130";
	...
	public int execute(Map request) throws Exception {
		...
		try {
			...
			if( request.size() > 1 ) {
				...
				eDate = (String)request.get("args2");
				sOrgid = (String)request.get("args3");
				log.debug(sDate + ".sDate");
				log.debug(eDate + ".eDate");
				log.debug(sOrgid + ".sOrgid");
			}
			...
			requestMap.put("OP_EDDT", eDate);
			requestMap.put("OP_ORGID", sOrgid);
			requestMap.put("OP_USER", PROG_ID.substring(0, 10));
			// 프로시저 호출
			if(log.isDebugEnabled()){
				log.debug("ACCDCL08130.execute.callSP_ACCDCL08130");
				log.debug("requestMap.toString()"+requestMap.toString());
			}
			sqlMap.queryForObject("ACCDCL08130.callSP_ACCDCL08130", requestMap);
