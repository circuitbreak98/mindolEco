...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 단말기변경통계</li>
 ...
 */
public class SALSTA08040 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08040";
	private static final String USER_ID = "SALSTA0804";
	private static String fileName = "";
	private static String ukeyIfType = "A2";
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fSubCd         = "";
		String fStrdDt        = "";
		String fGnrlChgAllotCncl       = "";
		String fGnrlChgAllot       = "";
		String fGnrlChgCash    = "";
		String fCmpChg      = "";
		String fLegalChg      = "";
		String fCmpChgCncl  = "";
...
/*
		...
		resultDeal      = (Map)sqlMap.queryForObject("SALSTA08040.getYesterDay", requestMap);
...
*/
		...
		sqlMap.update("SALSTA08040.deleteUkeyEqpChange", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fGnrlChgAllot     = readLine.substring( 20,  31).trim();
					fGnrlChgAllotCncl = readLine.substring( 31,  42).trim();
					fGnrlChgCash      = readLine.substring( 42,  53).trim();
					fCmpChg           = readLine.substring( 53,  64).trim();
					fLegalChg         = readLine.substring( 64,  75).trim();
					fCmpChgCncl       = readLine.substring( 75,  86).trim();
					fEqpChgTotal      = readLine.substring( 86,  97).trim();

					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08040.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08040.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("GNRL_CHG_ALLOT_CNT",      fGnrlChgAllot);
	            	requestMapUpd.put("GNRL_CHG_ALLOT_CNCL_CNT", fGnrlChgAllotCncl);
	            	requestMapUpd.put("GNRL_CHG_CASH_AGMRT_CNT", fGnrlChgCash);
	            	requestMapUpd.put("CMP_CHG_CNT",             fCmpChg);
                    requestMapUpd.put("LEGAL_CHG_CNT",           fLegalChg);
	              	requestMapUpd.put("CMP_CHG_CNCL_CNT",        fCmpChgCncl);
	            	requestMapUpd.put("EQP_CHG_TOTAL_CNT",       fEqpChgTotal);
	            	requestMapUpd.put("USER_ID",                 USER_ID);

	            	sqlMap.update("SALSTA08040.addUkeyEqpChange", requestMapUpd);
