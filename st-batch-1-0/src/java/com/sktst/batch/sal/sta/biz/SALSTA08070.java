...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 위성DMB패키지통계</li>
 ...
 */
public class SALSTA08070 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08070";
	private static final String USER_ID = "SALSTA0807";
	private static String fileName = "";
	private static String ukeyIfType = "A6";
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fPackProdId   = "";
		String fPackScrbNew  = "";
		String fPackScrbCnvt = "";
		String fPackScrbTot  = "";
		String fPackTermGnrl = "";
		String fPackTermAuth = "";
		String fPackTermCnvt = "";
		String fPackTermTot  = "";
		String fPackSummary  = "";
		...
		sqlMap.update("SALSTA08070.deleteUkeyErddmbPackage", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fPackCl       = readLine.substring(  20,  21).trim();
					fPackProdId   = readLine.substring(  21,  31).trim();
					fPackScrbNew  = readLine.substring(  31,  42).trim();
					fPackScrbCnvt = readLine.substring(  42,  53).trim();
					fPackScrbTot  = readLine.substring(  53,  64).trim();
					fPackTermGnrl = readLine.substring(  64,  75).trim();
					fPackTermAuth = readLine.substring(  75,  86).trim();
					fPackTermCnvt = readLine.substring(  86,  97).trim();
					fPackTermTot  = readLine.substring(  97, 108).trim();
					fPackSummary  = readLine.substring( 108, 119).trim();

					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08070.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08070.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("PACK_CL",             fPackCl);
	            	requestMapUpd.put("PACK_PROD_ID",        fPackProdId);
	            	requestMapUpd.put("PACK_SCRB_NEW_CNT",   fPackScrbNew);
	            	requestMapUpd.put("PACK_SCRB_CNVT_CNT",  fPackScrbCnvt);
                    requestMapUpd.put("PACK_SCRB_TOT_CNT",   fPackScrbTot);
	              	requestMapUpd.put("PACK_TERM_GNRL_CNT",  fPackTermGnrl);
	            	requestMapUpd.put("PACK_TERM_AUTH_CNT",  fPackTermAuth);
	            	requestMapUpd.put("PACK_TERM_CNVT_CNT",  fPackTermCnvt);
	            	requestMapUpd.put("PACK_TERM_TOT_CNT",   fPackTermTot);
	            	requestMapUpd.put("PACK_SUMMARY_CNT",    fPackSummary);
	            	requestMapUpd.put("USER_ID",             USER_ID);

	            	sqlMap.update("SALSTA08070.addUkeyErddmbPackage", requestMapUpd);
