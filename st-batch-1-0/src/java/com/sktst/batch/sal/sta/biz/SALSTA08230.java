...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - B전화실적 통계</li>
 ...
 */
public class SALSTA08230 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08230";
	private static final String USER_ID = "SALSTA0823";
	private static String fileName = "";
	private static String ukeyIfType = "B3";
	...
	public int execute(Map request) throws Exception {
		...
		try {
            ...
            if (ukeyIfType.equals(fileName.substring(6, 8) )) {
            	...
            	logMng.writeLogFile("Correct IF Type is 'B3'.. But '" + fileName.substring(6, 8) + "' type accepted...");
            }
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fSubCd          		= "";
		String fStrdDt         		= "";
		String fWphonProdKndCd 		= "";
		String fReSaleCoClCd   		= "";
		String fSnglCombClStcCd		= "";
		String fApplCnt        		= "";
		String fApplTdaySvcCnt 		= "";
		String fSumApplTdayCnt    	= "";
		String fApplCnclCnt       	= "";
		String fNum070NewSvcCnt   	= "";
		String fKtskNpSvcCnt      	= "";
		String fLgskNpSvcCnt      	= "";
		String fEtcNpSvcCnt       	= "";
		String fSumNpSvcCnt       	= "";
		String fSum070NpSvcCnt    	= "";
		...
		sqlMap.update("SALSTA08230.deleteUkeyBphonAchi", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fWphonProdKndCd 	= readLine.substring(20, 21).trim();
					fReSaleCoClCd   	= readLine.substring(21, 22).trim();
					fSnglCombClStcCd    = readLine.substring(22, 23).trim();
					fApplCnt            = readLine.substring(23, 34).trim();
					fApplTdaySvcCnt     = readLine.substring(34, 45).trim();
					fSumApplTdayCnt     = readLine.substring(45, 56).trim();
					fApplCnclCnt        = readLine.substring(56, 67).trim();
					fNum070NewSvcCnt    = readLine.substring(67, 78).trim();
					fKtskNpSvcCnt       = readLine.substring(78, 89).trim();
					fLgskNpSvcCnt       = readLine.substring(89, 100).trim();
                    fEtcNpSvcCnt    	= readLine.substring(100, 111).trim();
                    fSumNpSvcCnt   		= readLine.substring(111, 122).trim();
					fSum070NpSvcCnt 	= readLine.substring(122, 133).trim();

					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08230.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08230.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("WPHON_PROD_KND_CD",      fWphonProdKndCd 	);
	            	requestMapUpd.put("SNGL_COMB_CL_STC_CD",    fSnglCombClStcCd   	);
	            	requestMapUpd.put("APPL_CNT",              	fApplCnt           	);
	            	requestMapUpd.put("APPL_TDAY_SVC_CNT",      fApplTdaySvcCnt    	);
	            	requestMapUpd.put("SUM_APPL_TDAY_CNT",      fSumApplTdayCnt    	);
	            	requestMapUpd.put("APPL_CNCL_CNT",          fApplCnclCnt       	);
	            	requestMapUpd.put("NUM_070_NEW_SVC_CNT",    fNum070NewSvcCnt   	);
	            	requestMapUpd.put("KTSK_NP_SVC_CNT",        fKtskNpSvcCnt      	);
	            	requestMapUpd.put("LGSK_NP_SVC_CNT",        fLgskNpSvcCnt      	);
	            	requestMapUpd.put("ETC_NP_SVC_CNT",         fEtcNpSvcCnt       	);
	            	requestMapUpd.put("SUM_NP_SVC_CNT",         fSumNpSvcCnt       	);
	            	requestMapUpd.put("SUM_070_NP_SVC_CNT",     fSum070NpSvcCnt    	);

	            	requestMapUpd.put("USER_ID",              USER_ID);

	            	sqlMap.update("SALSTA08230.addUkeyBphonAchi", requestMapUpd);
