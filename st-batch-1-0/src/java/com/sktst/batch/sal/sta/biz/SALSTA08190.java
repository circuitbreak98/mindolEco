...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 더블할인 상세신규</li>
 ...
 */
public class SALSTA08190 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08190";
	private static final String USER_ID = "SALSTA0819";
	private static String fileName = "";
	private static String ukeyIfType = "A9";
	...
	public int execute(Map request) throws Exception {
		...
		try {
            ...
            if (ukeyIfType.equals(fileName.substring(6, 8) )) {
            	...
            	logMng.writeLogFile("Correct IF Type is 'A9'.. But '" + fileName.substring(6, 8) + "' type accepted...");
            }
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fTkGnrlScrbCnt       = "";
		String fTkKtfsktNpScrbCnt   = "";
		String fTkLgtsktNpScrbCnt   = "";
		String fTkSumCnt            = "";
		String fThGnrlScrbCnt       = "";
		String fThKtfsktNpScrbCnt   = "";
		String fThLgtsktNpScrbCnt   = "";
		String fThSumCnt            = "";
		String fTtGnrlScrbCnt       = "";
		String fTtKtfsktNpScrbCnt   = "";
		String fTtLgtsktNpScrbCnt   = "";
		String fTtSumCnt            = "";
		String fM12AgrmtCnt         = "";
		String fM18AgrmtCnt         = "";
		String fM24AgrmtCnt         = "";
		...
		sqlMap.update("SALSTA08190.deleteUkeySuplProd", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fTkGnrlScrbCnt      = readLine.substring(21, 32).trim();
					fTkKtfsktNpScrbCnt  = readLine.substring(32, 43).trim();
					fTkLgtsktNpScrbCnt  = readLine.substring(43, 54).trim();
					fTkSumCnt           = readLine.substring(54, 65).trim();
					fThGnrlScrbCnt      = readLine.substring(65, 76).trim();
					fThKtfsktNpScrbCnt  = readLine.substring(76, 87).trim();
					fThLgtsktNpScrbCnt  = readLine.substring(87, 98).trim();
					fThSumCnt           = readLine.substring(98, 109).trim();
					fTtGnrlScrbCnt      = readLine.substring(109, 120).trim();
					fTtKtfsktNpScrbCnt  = readLine.substring(120, 131).trim();
					fTtLgtsktNpScrbCnt  = readLine.substring(131, 142).trim();
					fTtSumCnt           = readLine.substring(142, 153).trim();
					fM12AgrmtCnt        = readLine.substring(153, 164).trim();
					fM18AgrmtCnt        = readLine.substring(164, 175).trim();
					fM24AgrmtCnt        = readLine.substring(175, 186).trim();
					fM36AgrmtCnt        = readLine.substring(186, 197).trim();
					fM42AgrmtCnt        = readLine.substring(197, 208).trim();
					fM48AgrmtCnt        = readLine.substring(208, 219).trim();
					fM54AgrmtCnt        = readLine.substring(219, 230).trim();
					
					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08190.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08190.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("TK_GNRL_SCRB_CNT",       fTkGnrlScrbCnt      );
	            	requestMapUpd.put("TK_KTFSKT_NP_SCRB_CNT",  fTkKtfsktNpScrbCnt  );
	            	requestMapUpd.put("TK_LGTSKT_NP_SCRB_CNT",  fTkLgtsktNpScrbCnt  );
	            	requestMapUpd.put("TK_SUM_CNT",             fTkSumCnt           );
	            	requestMapUpd.put("TH_GNRL_SCRB_CNT",       fThGnrlScrbCnt      );
	            	requestMapUpd.put("TH_KTFSKT_NP_SCRB_CNT",  fThKtfsktNpScrbCnt  );
	            	requestMapUpd.put("TH_LGTSKT_NP_SCRB_CNT",  fThLgtsktNpScrbCnt  );
	            	requestMapUpd.put("TH_SUM_CNT",             fThSumCnt           );
	            	requestMapUpd.put("TT_GNRL_SCRB_CNT",       fTtGnrlScrbCnt      );
	            	requestMapUpd.put("TT_KTFSKT_NP_SCRB_CNT",  fTtKtfsktNpScrbCnt  );
	            	requestMapUpd.put("TT_LGTSKT_NP_SCRB_CNT",  fTtLgtsktNpScrbCnt  );
	            	...
	            	sqlMap.update("SALSTA08190.addUkeyDblDtlRtn", requestMapUpd);
