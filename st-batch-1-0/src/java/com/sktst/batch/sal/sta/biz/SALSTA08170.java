...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 부가요금제통계</li>
 ...
 */
public class SALSTA08170 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08170";
	private static final String USER_ID = "SALSTA0817";
	private static String fileName = "";
	private static String ukeyIfType = "A7";
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fStrdDt         = "";
		String fSuplProdCd     = "";
		String fPrcplnScrbNew  = "";
		String fPrcplnScrbChg  = "";
		String fPrcplnScrbTday = "";
		String fPrcplnScrbTot  = "";
		String fTermGnrl       = "";
		String fTermAuth       = "";
		String fTermPrcpln     = "";
		String fTermTot        = "";
		...
		sqlMap.update("SALSTA08170.deleteUkeySuplProd", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fSuplProdCd     = readLine.substring(  20,  30).trim();
					fPrcplnScrbNew  = readLine.substring(  30,  41).trim();
					fPrcplnScrbChg  = readLine.substring(  41,  52).trim();
					fPrcplnScrbTday = readLine.substring(  52,  63).trim();
					fPrcplnScrbTot  = readLine.substring(  63,  74).trim();
					fTermGnrl       = readLine.substring(  74,  85).trim();
					fTermAuth       = readLine.substring(  85,  96).trim();
					fTermPrcpln     = readLine.substring(  96, 107).trim();
					fTermTot        = readLine.substring( 107, 118).trim();
					fPostBrIncre    = readLine.substring( 118, 129).trim();
                    fPrcplnIncre    = readLine.substring( 129, 140).trim();
                    fSummary        = readLine.substring( 140, 151).trim();

					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08170.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08170.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("SUPL_PROD_CD",         fSuplProdCd);
	            	requestMapUpd.put("PRCPLN_SCRB_NEW_CNT",  fPrcplnScrbNew);
	            	requestMapUpd.put("PRCPLN_SCRB_CHG_CNT",  fPrcplnScrbChg);
	            	requestMapUpd.put("PRCPLN_SCRB_TDAY_CNT", fPrcplnScrbTday);
                    requestMapUpd.put("PRCPLN_SCRB_TOT_CNT",  fPrcplnScrbTot);
	              	requestMapUpd.put("TERM_GNRL_CNT",        fTermGnrl);
	            	requestMapUpd.put("TERM_AUTH_CNT",        fTermAuth);
	            	requestMapUpd.put("TERM_PRCPLN_CNT",      fTermPrcpln);
	            	requestMapUpd.put("TERM_TOT_CNT",         fTermTot);
	            	requestMapUpd.put("POST_BR_INCRE_CNT",    fPostBrIncre);
	            	requestMapUpd.put("PRCPLN_INCRE_CNT",     fPrcplnIncre);
	            	requestMapUpd.put("SUMMARY_CNT",          fSummary);
	            	requestMapUpd.put("USER_ID",              USER_ID);

	            	sqlMap.update("SALSTA08170.addUkeySuplProd", requestMapUpd);
