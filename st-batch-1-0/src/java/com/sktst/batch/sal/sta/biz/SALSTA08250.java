...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 비M-BANK 단말기 판매통계</li>
 ...
 */
public class SALSTA08250 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08250";
	private static final String USER_ID = "SALSTA0825";
	private static String fileName = "";
	private static String ukeyIfType = "B5";
	...
	public int execute(Map request) throws Exception {
		...
		try {
            ...
            if (ukeyIfType.equals(fileName.substring(6, 8) )) {
            	...
            	logMng.writeLogFile("Correct IF Type is 'B5'.. But '" + fileName.substring(6, 8) + "' type accepted...");
            }
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		sqlMap.update("SALSTA08250.deleteUkeyBmbankEquip", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fNewScrbCnt         	= readLine.substring(20, 31).trim();
					fKtfsktNpScrbCnt    	= readLine.substring(31, 42).trim();
					fLgtsktNpScrbCnt        = readLine.substring(42, 53).trim();
					fSumScrbCnt             = readLine.substring(53, 64).trim();
					fCmpEquipChgCnt         = readLine.substring(64, 75).trim();
					fGnrlCellChgAllotCnt    = readLine.substring(75, 86).trim();
					fSumEquipCnt            = readLine.substring(86, 97).trim();
					fTotCnt                 = readLine.substring(97, 108).trim();


					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08250.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08250.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("SUM_EQUIP_CNT",              fSumEquipCnt           	);
	            	requestMapUpd.put("TOT_CNT",                    fTotCnt                	);

	            	requestMapUpd.put("USER_ID",              USER_ID);

	            	sqlMap.update("SALSTA08250.addUkeyBmbankEquip", requestMapUpd);
