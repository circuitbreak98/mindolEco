...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 서비스정지통계</li>
 ...
 */
public class SALSTA08240 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08240";
	private static final String USER_ID = "SALSTA0824";
	private static String fileName = "";
	private static String ukeyIfType = "B4";
	...
	public int execute(Map request) throws Exception {
		...
		try {
            ...
            if (ukeyIfType.equals(fileName.substring(6, 8) )) {
            	...
            	logMng.writeLogFile("Correct IF Type is 'B4'.. But '" + fileName.substring(6, 8) + "' type accepted...");
            }
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		sqlMap.update("SALSTA08240.deleteUkeySuspRsn", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fSuspRsnCd		= readLine.substring(20, 22).trim();
					fItem01   		= readLine.substring(22, 33).trim();
					fItem02       	= readLine.substring(33, 44).trim();
					fItem03       	= readLine.substring(44, 55).trim();
					fItem04       	= readLine.substring(55, 66).trim();
					fItem05       	= readLine.substring(66, 77).trim();
					fItem06       	= readLine.substring(77, 88).trim();
					fItem07       	= readLine.substring(88, 99).trim();
					fItem08       	= readLine.substring(99, 110).trim();
					fItem09       	= readLine.substring(110, 121).trim();
					fItem10       	= readLine.substring(121, 132).trim();


					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08240.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08240.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("SUSP_RSN_CD",    fSuspRsnCd);
	            	requestMapUpd.put("ITEM_01",       	fItem01   );
	            	requestMapUpd.put("ITEM_02",        fItem02   );
	            	requestMapUpd.put("ITEM_03",        fItem03   );
	            	requestMapUpd.put("ITEM_04",        fItem04   );
	            	requestMapUpd.put("ITEM_05",        fItem05   );
	            	requestMapUpd.put("ITEM_06",        fItem06   );
	            	requestMapUpd.put("ITEM_07",        fItem07   );
	            	requestMapUpd.put("ITEM_08",        fItem08   );
	            	requestMapUpd.put("ITEM_09",        fItem09   );
	            	requestMapUpd.put("ITEM_10",        fItem10   );

	            	requestMapUpd.put("USER_ID",              USER_ID);

	            	sqlMap.update("SALSTA08240.addUkeySuspRsn", requestMapUpd);
