...
/**
 * <li>업무 그룹명 : 프로젝트/SKTST/영업/U.Key통계 I/F - 단말기통계</li>
 ...
 */
public class SALSTA08030 extends AbsBatchJobBiz {

	private static final String PROG_ID = "SALSTA08030";
	private static final String USER_ID = "SALSTA0803";
	private static String fileName = "";
	private static String ukeyIfType = "A3";
	...
	private void openDataFileAddDB(SqlMapClient sqlMap) throws Exception {
		...
		String fScrbTot       = "";
		String fEqpIncreChg   = "";
		String fEqpIncreCnvt  = "";
		String fEqpIncreTot   = "";
		String fEqpDecreChg   = "";
		String fEqpDecreCnvt  = "";
		String fEqpDecreTot   = "";
		String fTermGnrl      = "";
		String fTermMnp       = "";
		String fTermAuth      = "";
		...
		sqlMap.update("SALSTA08030.deleteUkeyEquipment", requestMap);
		...
		try {
			...
			for (String readLine; (readLine = br.readLine()) != null;) {
				...
				try {
					...
					fEqpTyp        = readLine.substring(  20,  21).trim();
					fScrbNew       = readLine.substring(  21,  32).trim();
					fScrbMnp       = readLine.substring(  32,  43).trim();
					fScrbTot       = readLine.substring(  43,  54).trim();
					fEqpIncreChg   = readLine.substring(  54,  65).trim();
					fEqpIncreCnvt  = readLine.substring(  65,  76).trim();
					fEqpIncreTot   = readLine.substring(  76,  87).trim();
					fTermGnrl      = readLine.substring(  87,  98).trim();
					fTermMnp       = readLine.substring(  98, 109).trim();
					fTermAuth      = readLine.substring( 109, 120).trim();
					fTermTotal     = readLine.substring( 120, 131).trim();
					fEqpDecreChg   = readLine.substring( 131, 142).trim();
					fEqpDecreCnvt  = readLine.substring( 142, 153).trim();
					fEqpDecreTot   = readLine.substring( 153, 164).trim();

					// 대리점 정보 조회
					requestMap.put("UKEY_AGENCY_CD", fAgencyCd);
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08030.getDealAgencyInfo", requestMap);
					...
					requestMap.put("EFF_DT",         fStrdDt);
					resultDeal = (Map)sqlMap.queryForObject("SALSTA08030.getDealPlcInfo", requestMap);
	            	...
	            	requestMapUpd.put("EQP_TYP",            fEqpTyp);
	            	requestMapUpd.put("SCRB_NEW_CNT",       fScrbNew);
	            	requestMapUpd.put("SCRB_MNP_CNT",       fScrbMnp);
	            	requestMapUpd.put("SCRB_TOT_CNT",       fScrbTot);
	            	requestMapUpd.put("TERM_GNRL_CNT",      fTermGnrl);
                    requestMapUpd.put("TERM_MNP_CNT",       fTermMnp);
	              	requestMapUpd.put("TERM_AUTH_CNT",      fTermAuth);
	            	requestMapUpd.put("TERM_TOT_CNT",       fTermTotal);
	            	requestMapUpd.put("EQP_INCRE_CHG_CNT",  fEqpIncreChg);
	            	requestMapUpd.put("EQP_INCRE_CNVT_CNT", fEqpIncreCnvt);
	            	requestMapUpd.put("EQP_INCRE_TOT_CNT",  fEqpIncreTot);
	            	requestMapUpd.put("EQP_DECRE_CHG_CNT",  fEqpDecreChg);
	            	requestMapUpd.put("EQP_DECRE_CNVT_CNT", fEqpDecreCnvt);
	            	requestMapUpd.put("EQP_DECRE_TOT_CNT",  fEqpDecreTot);
	            	requestMapUpd.put("USER_ID",            USER_ID);

	            	sqlMap.update("SALSTA08030.addUkeyEquipment", requestMapUpd);
