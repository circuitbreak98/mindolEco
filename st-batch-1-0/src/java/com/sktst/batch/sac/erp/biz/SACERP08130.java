 ...
 */
public class SACERP08130 extends AbsBatchJobBiz {
	private static final String PROG_ID = "SACERP08130";
	...
	public int execute(Map request) throws Exception {
		...
		try {
			...
			if(accMth.compareTo("201005") < 0 || "03".equals(accMonth) || "06".equals(accMonth) || "09".equals(accMonth)|| "12".equals(accMonth)){
				budat = zbudat; // 전기일자
//			}else{
//			    if (  pZgubun.equals(ErpCommon.ZGUBUN_SALE_CMMS_SUB) 
//			       || pZgubun.equals(ErpCommon.ZGUBUN_FIX_VIRTL_ACC_CMMS) 
//	 		 	   || pZgubun.equals(ErpCommon.ZGUBUN_FIX_ACC_TRNSF_CMMS)
//				   || pZgubun.equals(ErpCommon.ZGUBUN_FIX_CARD_SETTLE_CMMS)
//				   || pZgubun.equals(ErpCommon.ZGUBUN_FIX_AGRMT_ASTAMT)
//				   || pZgubun.equals(ErpCommon.ZGUBUN_FIX_ALLOT_SALE_AMT) ) {
//					budat = zbudat; // 전기일자
//				    }
			}
			 ...
			 *    대리점 조회 
			 */
			lstAgencyList = (List)sqlMap.queryForList("SACERP08130.getAgencyList", requestMap);
	...
	private void sendSaleCmms(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getCmmsAccList", requestMap);
				...
				for (int j = 0; j < lstJourStdInfo.size(); j++) {
					...
					if(ukeySubCdMap != null){
						...
						if(!"0000".equals(ukeySubCd)){
							mCmmsAcc.put("UKEY_SUB_CD", ukeySubCd);
						}
					}
			...
			sqlMap.update("SACERP08130.updateCmmsAccSendInfo"    , mCmmsAcc);  /* 수수료_월정산             */
			sqlMap.update("SACERP08130.updateOaCmmsAccSendInfo"  , mCmmsAcc);  /* 도매 수수료_월정산  */
			sqlMap.update("SACERP08130.updateOsCmmsAccSendInfo"  , mCmmsAcc);  /* 소매 수수료_월정산  */
	...
	private void sendSaleCmmsAdd(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getAccMtchAmtAddList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
			...
			mCmmsAcc.put("ZCONFIRM", mKey.get("ZCONFIRM"));
			sqlMap.update("SACERP08130.updateCmmsAccSendInfo"    , mCmmsAcc);  /* 수수료_월정산             */
			sqlMap.update("SACERP08130.updateMcCmmsAccSendInfo"  , mCmmsAcc);  /* MC채널 수수료_월정산  */
			sqlMap.update("SACERP08130.updateOtlCmmsAccSendInfo" , mCmmsAcc);  /* 결제 수수료_월정산  */
			sqlMap.update("SACERP08130.updateOaCmmsAccSendInfo"  , mCmmsAcc);  /* 도매 수수료_월정산  */
			sqlMap.update("SACERP08130.updateOsCmmsAccSendInfo"  , mCmmsAcc);  /* 소매 수수료_월정산  */
	...
	private void sendSaleCmmsSub(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getAccMtchAmtSubList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
	...
	private void sendFixVirtlAccCmms(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getFixVirtlAccCmmsList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
...
//				}
	...
	private void sendFixAccTrnsfCmms(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getFixAccTrnsfCmmsList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
...
//					logMng.writeLogFile("대리점[" + agency + "] : [정산처[" + accPlc + "] : 확정계좌이체수수료[fix_acc_trnsf_cmms] : " + fixAccTrnsfCmms.toString());
//				}
	...
	private void sendFixCardSettleCmms(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getFixCardSettleCmmsList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
...
//				}
	...
	private void sendMcSaleCmms(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getMcCmmsAccList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
...
////				}
	...
	private void sendFixAgrmtAstAmt(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getFixAgrmtAstAmtList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
...
//				}
	...
	private void sendFixAllotSaleAmt(SqlMapClient sqlMap) throws Exception {
		...
		for (int idx = 0; idx < lstAgencyList.size(); idx++) {
			...
			requestMap.put("AGENCY", agency);
			requestMap.put("FIX_DT", fixDt);
			lstAmtInfo = (List)sqlMap.queryForList("SACERP08130.getFixAllotSaleAmtList", requestMap);
			if ( lstAmtInfo.size() == 0 ) {
